...
// Experiment 05: Word wheel — scroll wheel triggers smooth morph.
//
// Same particle field + same morph mechanics as Experiment 03 (v2). The
// trigger is a `wheel` event instead of a `mouseenter`. Wheel down (deltaY
// positive) advances to the next phrase. Wheel up (deltaY negative) goes
// to the previous one. Each trigger fires a 4-second smoothstep morph;
// rapid wheel ticks throttle so we don't chain too fast.
//
// Note: we preventDefault on the wheel event so scrolling on this canvas
// drives the phrase change rather than scrolling the page. To scroll past
// this lab section, scroll OUTSIDE the canvas area (left/right gutters)
// or scroll over a different section.
...
function WheelField({ phraseAnchors }: { phraseAnchors: Float32Array[] }) {
  ...
  function fireMorph(direction: 1 | -1) {
    ...
    const len = phraseAnchors.length;
    const nextIdx = (currentIdxRef.current + direction + len) % len;
    ...
    <section className="relative h-screen w-full overflow-hidden border-b border-bone/5 bg-ink">
      {phraseAnchors && (
        ...
        >
          ...
          <ScreenPixelCamera />
          <WheelField phraseAnchors={phraseAnchors} />
          <EffectComposer>
            <Bloom
              intensity={1.1}
